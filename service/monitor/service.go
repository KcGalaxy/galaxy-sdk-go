...
// Monitor provides the API operation methods for making requests to
// monitor. See this package's package overview docs
// for details on the service.
//
// Monitor methods are safe to use concurrently. It is not safe to
...
//     svc := monitor.New(mySession)
//
//     // Create a Monitor client with additional configuration
//     svc := monitor.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
func New(p client.ConfigProvider, cfgs ...*aws.Config) *Monitor {
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Monitor {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Monitor {
	_info := utils.UrlInfo{
		UseSSL:      false,
		Locate:      false,
		UseInternal: false,
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Monitor {
	svc := &Monitor{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Monitor operation and runs any
// custom request initialization.
func (c *Monitor) newRequest(op *request.Operation, params, data interface{}) *request.Request {
