...
// Dedicated provides the API operation methods for making requests to
// dedicated. See this package's package overview docs
// for details on the service.
//
// Dedicated methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the Dedicated client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Dedicated client from just a session.
//     svc := dedicated.New(mySession)
//
//     // Create a Dedicated client with additional configuration
//     svc := dedicated.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Dedicated {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Dedicated {
	_info := utils.UrlInfo{
		UseSSL:      false,
		Locate:      false,
		UseInternal: false,
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Dedicated {
	svc := &Dedicated{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Dedicated operation and runs any
// custom request initialization.
func (c *Dedicated) newRequest(op *request.Operation, params, data interface{}) *request.Request {
