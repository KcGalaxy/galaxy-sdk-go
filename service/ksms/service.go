...
// ksms. See this package's package overview docs
// for details on the service.
//
// Ksms methods are safe to use concurrently. It is not safe to
...
const (
	ServiceName = "ksms"      // Name of service.
	EndpointsID = ServiceName // ID to lookup a service endpoint with.
	ServiceID   = "ksms"      // ServiceID is a unique identifer of a specific service.
)

// New creates a new instance of the Ksms client with a session.
// If additional configuration is needed for the client instance use the optional
// ksc.Config parameter to add your extra config.
//
// Example:
//     // Create a Ksms client from just a session.
//     svc := ksms.New(mySession)
//
//     // Create a Ksms client with additional configuration
//     svc := ksms.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func ExtraNew(info *utils.UrlInfo, p client.ConfigProvider, cfgs ...*aws.Config) *Ksms {
	c := p.ClientConfig(EndpointsID, cfgs...)
...
func SdkNew(p client.ConfigProvider, cfgs *ksc.Config, info ...*utils.UrlInfo) *Ksms {
	_info := utils.UrlInfo{
		UseSSL:      false,
		Locate:      false,
		UseInternal: false,
...
func newClient(cfg aws.Config, handlers request.Handlers, endpoint, signingRegion, signingName string) *Ksms {
	svc := &Ksms{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a Ksms operation and runs any
// custom request initialization.
func (c *Ksms) newRequest(op *request.Operation, params, data interface{}) *request.Request {
